#!/usr/bin/env bash
# ==========================================================================
# 04-enum-web.sh  -  Probe, fingerprint, screenshot, content-discover, and
# scan all HTTP/S services found externally (by IP and by discovered hostname).
# ==========================================================================
set -euo pipefail
cd "$(dirname "$0")"
source ./config.sh
...
# Build the working URL set: scanned IP:port web services PLUS discovered
# hostnames (web apps are usually name-vhosted). httpx then confirms liveness.
build_targets() {
  { [[ -s "$URLS" ]] && cat "$URLS"; } 2>/dev/null
  if [[ -s "$RUN/02-osint/subdomains.txt" ]]; then
    while read -r h; do
      [[ -n "$h" ]] || continue
      printf 'http://%s\nhttps://%s\n' "$h" "$h"
    done < "$RUN/02-osint/subdomains.txt"
  fi
}

if ! task_listing; then
  build_targets | sort -u > "$OUT/targets_all.txt"
  ...
  phase "Web Enumeration ($(wc -l <"$LIVE_URLS") live targets)"
fi
...
t_exposures() {
  log "Exposure checks (.git/.svn/.env, backups, status, secrets, cloud meta)"
  local EXPOSE_PATHS=(
    ...
    .well-known/security.txt actuator/env actuator/health actuator
    swagger.json api/swagger.json openapi.json .aws/credentials
    wp-config.php.bak .git-credentials id_rsa
  )
  ...
  if [[ -s "$OUT/exposures.txt" ]]; then
    ...
    warn "Exposed paths -> $OUT/exposures.txt ($(_ai_count "$OUT/exposures.txt"))"
  fi
}

# ---- Sub-task: WordPress passive deep-scan --------------------------------
t_wpscan() {
...
t_vhost() {
  { have ffuf && [[ -n "${VHOST_WORDLIST:-}" && -s "${VHOST_WORDLIST:-/nonexistent}" && -n "${TARGET_DOMAIN:-}" ]]; } \
    || { warn "ffuf/VHOST_WORDLIST/TARGET_DOMAIN not all set — skipping vhost discovery."; return 0; }
  log "vhost discovery against ${TARGET_DOMAIN}"
  local base safe
  while read -r base; do
    safe=$(sed 's#[^A-Za-z0-9]#_#g' <<<"$base")
    ffuf -u "$base" -H "Host: FUZZ.${TARGET_DOMAIN}" -w "$VHOST_WORDLIST" \
...
t_crawl() {
  ...
  if have feroxbuster && [[ -s "$WEB_WORDLIST" ]]; then
    ...
    local THREADS_WEB=$(( THREADS<6 ? THREADS : 6 ))
  ...
  if have python3; then
    ...
    if [[ -s "$OUT/filtered_urls.txt" ]]; then
      ...
      ok "Consolidated nuclei targets: $(_ai_count "$OUT/nuclei_targets.txt") (roots + discovered, de-noised)"
    fi
  fi
}
...
t_nuclei() {
  ...
  run "$LOG" nuclei -l "$targets" \
    ...
    -rl "${NUCLEI_RATELIMIT:-100}" -c 25 -o "$OUT/nuclei.txt" -stats 2>/dev/null || true
  ...
  # tech stack the AI inferred from the fingerprints (ai_bridge_04). Tags are
  # re-sanitised here ([a-z0-9_-] only) before touching the command.
  if [[ "${AI_NUCLEI_TAGS:-true}" == "true" && -s "$OUT/ai_nuclei_tags.txt" ]]; then
    ...
    if [[ -n "$t" ]]; then
      ...
      run "$LOG" nuclei -l "$targets" -tags "$t" \
        -timeout "${NUCLEI_TIMEOUT:-10}" -retries 1 \
        -rl "${NUCLEI_RATELIMIT:-100}" -c 25 -o "$OUT/nuclei_ai.txt" -stats 2>/dev/null || true
...
task exposures   "Exposure checks (.git/.env/backups/status/cloud)"   t_exposures
