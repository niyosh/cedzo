...
[[ -s "$DBH" ]] || { warn "No DB hosts found. Skipping."; exit 0; }

phase "Database Enumeration"
NXC=$(nxc_bin) || NXC=""

# Re-scan DB hosts with DB-specific NSE for a quick characterisation.
...
# MSSQL via netexec — windows-auth and local-auth probes.
if [[ -n "$NXC" ]]; then
  log "MSSQL probe (nxc) — null + provided creds"
  run "$LOG" "$NXC" mssql "$DBH" || true
  if [[ -n "$USERNAME" ]]; then
    CRED=(-u "$USERNAME"); [[ -n "$NTHASH" ]] && CRED+=(-H "$NTHASH") || CRED+=(-p "$PASSWORD")
...
# Default-credential sweep is high-value on internal DBs. Conservative list.
